...
#          ⚠️ **包的指纹会随重建而失效，下面这组数字只描述「当时那一个包」**：
#             · 第一次（02:52 构建）：3,691,396 B，sha256 `fcb51f2ebb3b…fda66`；
#             · 第二次（03:48 构建）：3,691,588 B，sha256 `098a6d2e5533…d298b`。
#             两个包**各重跑过一次，均 exit 0**（遵守上面「换包后必须重跑」）。
#          ⇒ **核对现况请以 `sha256sum <该路径>` 的当场结果为准，别拿本注释当现况**：
#             每重建一次哈希就换一次，照注释里的旧哈希去找会找不到，然后白白怀疑自己
#             —— 第一次那个指纹就是这么飘掉的（由 tessa-smoke3 实测指出）。
...
#             · **样本数 = 2**（两个 release 包各一次），仍不具统计意义，
#               换包后必须重跑（规矩同上：行号是可机读的取证锚点）；
#             · **CI 上的 release job 已不再「一次都没跑过」，但成功次数仍是 0** ——
#               实测 run 35466724544：`四闸门 = success` ⇒ 该 job **首次真正执行**，
#               结果是 **failure**（第 10 步产物收集路径写错，已由 `791128d` 修正）。
#               ⚠️ 注意它与早先两次的区别：那两次是 **skipped**（`needs: gates` 上游红，
#               不红不报警、完全不可见），这一次是 **failure**（可见）。ci.yml 已新增
#               哨兵 job 把「静默跳过」变成可见失败 —— 但哨兵只解决「不可见」，
#               **不解决「还没跑通过」**；
