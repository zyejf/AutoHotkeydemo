...
class IpcClient {
    ...
    static _SendMsg(msg) {
        ...
        ; -----------------------------------------------------------------
        ; A-4 技术债登记（2026-09-12 审查结论，暂不改造）
        ; -----------------------------------------------------------------
        ; 本处 WriteFile 为「同步阻塞写」（第 5 参数 overlapped 传 Ptr 0）。
        ; 管道对端处理慢时，AHK 钩子线程会被阻塞，理论上存在录制丢键风险。
        ;
        ; 为何暂不改为异步（FILE_FLAG_OVERLAPPED 常量已在类静态成员中定义）：
        ;   1. 影响面仅限「录制/验证模式」—— 普通回放模式的热路径用 SendInput
        ;      本地调用，完全不含 IPC（见 sender.ahk 的 _ReportKeyEvent 门控）；
        ;   2. 改为 OVERLAPPED 需配套改造：将管道创建改为 overlapped 模式、
        ;      引入 OVERLAPPED 结构体与事件对象、实现写完成轮询/回调，
        ;      并处理「缓冲区在写完成前不得复用」的生命周期问题。
        ;      对 AHK 单线程钩子模型而言，这相当于重做 IPC 并发模型，
        ;      收益/风险比不佳；
        ;   3. 目前无录制丢键的实测证据。
        ;
        ; 触发重新评估的条件（满足任一即应实施 OVERLAPPED 改造）：
        ;   - 出现录制丢键的实测证据（如按键计数与实际不匹配）；
        ;   - 录制模式改为默认开启，使其进入常规使用路径。
        ; 详见 docs/review/2026-09-12/issue-inventory.md 的「技术债登记」。
        ; -----------------------------------------------------------------
        bytesWritten := 0
