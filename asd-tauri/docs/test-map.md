...
| asd-application | 单元 | crates/asd-application/src/config_repository.rs | 26 | ConfigRepository 文件 I/O 与序列化 + **TD-027 新增 2 条**：文件存在但读不出来（非 UTF-8 / 路径是目录）必须报 `IoError` 且消息里**不得**出现「不存在」 |
...
自洽校验：各 crate「小计 = 明细之和」，上表 Rust 总数 = asd-domain + asd-ipc-protocol + asd-application + asd-test-harness + asd-tauri = 154 + 72 + 168 + 3 + 242 = 639。
