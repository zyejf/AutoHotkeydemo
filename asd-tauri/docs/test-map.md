...
> - **2026-09-20 补登记（此前漏登，G3d 因此红）**：`infrastructure/watchdog.rs` 的 6 条 spawn 重试策略用例（随 `b758f23` 落地未登记，38 → 44）与 `src-tauri/src/tests/ahk_executor_path_tests.rs` **整行 6 条**（随 `0b2f4f9` 落地未登记），再加本轮新增 1 条（`retry_never_fires_when_executor_missing_at_any_attempt`，44 → 45）。合计 **+13**：asd-tauri 260 → **273**、Rust 总数 673 → **686**。教训：**新增测试必须与本文档同批登记**，否则 G3d 会在几天后以一个「凭空多出来的数字」爆出来，且与真正触发它的代码改动不同批、极难归因。
...
| Crate | 类型 | 文件路径 | 测试数 | 覆盖范围 |
|-------|------|---------|-------|---------|
| asd-tauri | 单元 | src-tauri/src/infrastructure/ipc.rs | 57 | IpcManager named pipe 通信。**2026-09-19 实测 52 → 54（+2）→ 57（+3，TD-071 IPC 管道 DACL 收紧，Cody）**：① `test_ipc_pipe_sddl_trustee_is_real_current_user_sid` —— SDDL 里的 trustee 必须是**当前用户的真实 SID**，不得是硬编码/占位 SID；② `test_security_descriptor_is_actually_enforced` —— 安全描述符必须**真的生效**（同用户可连、非授权不可连的正反双向），防「拼出了 DACL 字符串却没应用」；③ `test_ipc_listener_accepts_same_user_connection` —— 监听器须接受同用户连接（回归防线，防 DACL 收紧过头把正常路径一起挡掉）。另 **2026-09-19 实测 52 → 54（+2）**：① `test_ipc_pipe_name_is_per_session_and_stable` —— 管道名须以 `IPC_PIPE_NAME_BASE` 开头、不等于基础名、含当前 PID、同进程内稳定（对应 `IPC_PIPE_NAME` 拆为 `IPC_PIPE_NAME_BASE` + `IPC_PIPE_NAME_ENV_VAR` + `ipc_pipe_name()` 的每会话化改造）；② `test_create_listener_reports_name_conflict` —— 同名管道二次 `create_listener` 须返回 `Err`（抢注可检出的阳性对照）。另 `test_ipc_pipe_name_constant_value` / `test_ipc_pipe_name_matches_ahk_client` 属**改写非新增**，后者现同时校验「AHK 回退基础名」与「EnvGet 变量名」两维 |
| asd-tauri | 单元 | src-tauri/src/infrastructure/watchdog.rs | 45 | ProcessWatchdog + WatchdogRunner 进程管理。**2026-09-19 实测 32 → 38（+6，Cody 批次）**：① `test_cleanup_does_not_kill_unregistered_same_name_process` —— 未登记的同名进程**必须存活**（核心阳性对照，旧行为是被杀）；② `test_terminate_registered_child_kills_it` —— 登记过的必须真被终止（反向对照，证明清理能力未失效）；③ `test_is_terminable_child_excludes_self_pid`；④ `test_cleanup_drains_registry`；⑤ `test_expected_image_name_variants`；⑥ `test_no_image_name_based_taskkill_in_source`（静态禁源码出现 `"/IM"`）。①②会**真实起进程**（把 `cmd.exe` 复制到 temp 改名 `asd_executor.exe`，跑完 kill + 删目录；取不到 `SystemRoot\System32\cmd.exe` 则打印「跳过」并 return，不假绿也不假红），两条合计约 1s。**2026-09-20 实测 38 → 45（+7）**：其中 **+6 属此前漏登记**（启动期 spawn 重试策略 `decide_spawn_retry` 的 6 条纯函数用例 —— 缺失恒 GiveUp／存在则重试／退避翻倍／到上限放弃／预算恰为 `MAX-1`／累计延迟 <1s，随 `b758f23` 落地但当时未同步本文档），**+1 是本轮新增** `retry_never_fires_when_executor_missing_at_any_attempt`（`exe_exists=false` 时对**任意** attempt 序号都必须 GiveUp；变异验证：把门控改成 `if false` ⇒ 2 条红、rc=101） |
| asd-tauri | 单元 | src-tauri/src/tests/ahk_executor_path_tests.rs | 6 | **TD-089 ① 新增（`0b2f4f9` 落地，2026-09-20 补登记 —— 此前整行缺失）**：`format_resource_snapshot` 两级快照的纯函数单测 —— ① 目录与文件都在时两级都报；② 区分「文件缺」与「目录缺」；③ 目录缺时不列举；④ 父目录无法推导（`Path::new("x.exe").parent()` 返回 `Some("")`）不得误报成「目录存在=false」；⑤ 列举有上界并标注截断；⑥ 快照**永不为空串**（变异验证：返回值改成空串 ⇒ 全红 rc=101）。⚠️ 覆盖的是**格式化函数本体**，两处**调用点的接线**仍无测试 |
...
> 注：`--lib` 明细之和 259 与 `cargo test -p asd-tauri --lib -- --list` 运行时注册数一致；
> benches 使用 criterion（`harness = false`），在 `--all-targets -- --list` 下**不注册**为测试，
> 故 `--all-targets` = 259（lib）+ 1（tests/test_manifest_feature_removed.rs）= **260**，
...
自洽校验：各 crate「小计 = 明细之和」，上表 Rust 总数 = asd-domain + asd-ipc-protocol + asd-application + asd-test-harness + asd-tauri = 161 + 72 + 177 + 3 + 273 = 686。

> **用例总数下限闸门（E 段，`scripts/check-test-map.py`）**：Rust 测试总数**只许增不许减**，下限钉在 **673**（容差 0；2026-09-19 实测水位）。任何一条测试以「挂 `#[ignore]` / `#[cfg]` 排除 / 未挂进编译 / 文件被删 / target 配置被改」任一方式消失，总数一降即红。抬高该下限是**显式的人的决定**，须在提交信息里说明新增了哪些测试。
...
| asd-tauri/src/__tests__/diagnostics_contract.test.js | **「诊断信息」页与 UI 事件接线的静态契约测试**（2026-09-19 新增，随 E3 一并落地）：**13 个用例**（2026-09-19 晚 9 → **13**，+4），锁四类不变量 —— ① `index.html` 的每个 `data-action` 都在 `main.js` 有分发分支（**防"按钮点了没反应"**，这类错没有任何报错、只表现为点击静默无效）；② 日志目录取 `appDataDir()` 而**不是** `appLogDir()`（与 Rust 侧实际落盘位置 `logging.rs:19-33` 的 `app.path().app_data_dir()` 对齐 —— 取错则诊断页给出打不开的路径，比不给更糟）；③ `index.html` 与 `main.js` 生成的 HTML 均无内联事件处理器（CSP `script-src 'self'` 无 `unsafe-inline` 的回归防线，即 #16 的硬指标）。**三条阳性对照均已实测变红并精确点名**（改 `appDataDir`→`appLogDir` / 把 `refreshDiagnostics` 改名 / 注入 `onclick=`），还原后回绿；④ **执行器失败态的自助恢复**（TD-082，+4 条）：`EXEC_STATUS_MAP` 的键必须与 Rust `WatchdogStateEnum` 的 **7 个变体**逐一对齐（多一个键是已删的过期状态、少一个键会把英文原文直接显示给用户）、诊断页须有 `diagExecStatus`/`diagExecRestart` 与 `data-action="resetWatchdog"`、`resetWatchdogDiag()` 必须真的调用 `api.resetWatchdog()`（该封装此前**一直零调用**，正是本条债的成因）、`onExecutorStatus` 在 Failed 时必须给 toast + 日志且**由「上一次状态」守卫**只提示一次。**阳性对照已实测**：把 `Hung` 改名为 `HungX` → 1 条红，还原后回绿 |
