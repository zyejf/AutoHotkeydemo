...
> - AHK 完整套件汇总：runner 除 `Test_` 方法外还执行 `Setup`/`Teardown` 生命周期钩子，故实跑总数（**638**）略高于静态 `Test_` 计数（**636**）。
...
| AHK | 单元 | tests/test_ahk_executor/test_sender.ahk | 13 / 47 | sender.ahk AllowedKeys、ValidateKey、ToggleGroup、StartPeriodic/Sequence/Enhanced/Hold、HoldModeToggle、EmergencyRelease、Shutdown、Init、ReportKeyEvents、**QPC 精确定刻（时钟分辨率／SleepUntil 误差／保持时长对齐 kpd／端到端 P95 ≤ 20ms／零漏发）** |
...
| AHK 执行器测试（`Test_` 方法数） | 60 套件 / 260 个 `Test_` 方法（`tests/test_ahk_executor/` 5 文件；不含 `test_joy_hotkey_manager_ahu.ahk` 的 7 套件） |
| AHK v2 完整测试套件（`tests/run_all_tests.ahk` 汇总） | 638 个用例（通过 638 / 失败 0），163 个套件 |
