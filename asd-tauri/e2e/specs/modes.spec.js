...
describe('7 种执行模式 E2E 测试', () => {
  ...
  beforeEach(function () {
    if (!binaryAvailable) {
      this.skip('Binary not found, skipping all modes E2E tests');
    }
  });
  ...
  afterEach(function () {
    ...
    if (!test) return;
    const suiteName = test.parent?.title || '7 种执行模式 E2E 测试';
    ...
    if (test.state === 'failed') {
      const caseId = test.title.match(/E2E-MODE-\d+/)?.[0] || test.title;
      appendKnownIssue(
        `ISSUE-${caseId}`,
        'HIGH',
        `执行测试用例 ${test.title}`,
