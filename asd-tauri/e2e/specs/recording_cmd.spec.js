...
describe('recording_cmd E2E 测试', () => {
  ...
  beforeEach(function () {
    if (!binaryAvailable) {
      this.skip('Binary not found, skipping all recording_cmd E2E tests');
    }
  });
  ...
  afterEach(async function () {
    ...
    if (!test) return;
    const suiteName = test.parent?.title || 'recording_cmd E2E 测试';
    ...
    if (test.state === 'failed') {
      const caseId = test.title.match(/E2E-REC-\d+/)?.[0] || test.title;
      appendKnownIssue(
        `ISSUE-${caseId}`,
        'HIGH',
        `执行测试用例 ${test.title}`,
        ...
        errorMsg,
        `E2E 测试失败: ${test.title}`,
        '检查录制状态机实现与 IPC 通信，或确认 E2E 环境是否支持 AHK 子进程',
        caseId
      );
    }
