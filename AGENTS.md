...
3. **定时器提前唤醒量必须 ≥ 一个网格周期 + 抖动余量**（`Sender.WAKE_LEAD_US = 28000`，即 28 ms）。
...
8. **调度时刻一律用「整数微秒」**（`HighResClock.NowUs()` / `SleepUntilUs()`），
   禁止用**浮点毫秒**做累加推进与相等比较。配置侧的间隔/延时在入口处用 `Round(x * 1000)`
   一次性转成整数 µs（`Sender._IntervalUsOf` / `_DelayUsOf`），之后全程整数运算。
   原因：浮点累加的末位误差会把「数学上同一时刻到期」的两个键判成两个不同的 Map 键，
...
| `docs/refactor/` | **重构方案与实测对比的唯一落点**（A/B/C 三份开发计划 + 方案 C 深度评测 + 优化前后基准报告 + 调度算法设计）。基准脚本在 `tools/ahk-bench/`（`lib/seqgen_test.ahk` 是原型自测，**不属于生产测试套件，不登记 test-map**） |
