; =================================================================
; 基础设施层 - JoystickInputUtils 手柄输入工具
; 版本: 1.0
; 说明: 从 domain/joystick_input.ahk 迁移的纯工具函数
;       供 infrastructure/joy_hotkey_manager.ahk 使用，避免反向依赖
;       定义手柄输入标识的解析、映射和验证规则
...
class JoystickInputUtils {
    static THRESHOLD := 80
    static DEADZONE := 20
    static HYSTERESIS := 10

    static IsJoystickKey(key) {
        return JoystickInputUtils.IsButton(key) || JoystickInputUtils.IsPov(key)
            || JoystickInputUtils.IsAxis(key) || JoystickInputUtils.IsTrigger(key)
    }
    ...
    static GetAhkReadKey(key) {
        if JoystickInputUtils.IsButton(key) || JoystickInputUtils.IsTrigger(key)
            return RegExReplace(key, "^(Joy\d+).*", "$1")
        if JoystickInputUtils.IsAxis(key) {
            axis := StrSplit(key, "_")[1]
            return axis
        }
        if JoystickInputUtils.IsPov(key)
            return "JoyPOV"
        return key
    }
