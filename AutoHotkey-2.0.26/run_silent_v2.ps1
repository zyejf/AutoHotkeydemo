...
$ErrorLog = Join-Path $ScriptDir "logs\$ScriptName`_error.log"
...
$process = Start-Process -FilePath $AHKPath -ArgumentList "/ErrorStdOut", $ScriptPath -RedirectStandardError $ErrorLog -Wait -NoNewWindow -PassThru

if (Test-Path $ErrorLog) {
    $error = Get-Content $ErrorLog -Raw
    if ($error -and $error.Trim() -ne "") {
