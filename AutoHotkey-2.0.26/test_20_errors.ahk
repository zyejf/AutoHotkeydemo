...
global LogFile := A_ScriptDir "\test_20_errors.log"
global Num := 0

if FileExist(LogFile)
    FileDelete(LogFile)

FileAppend("测试 20 种错误`n`n", LogFile, "UTF-8")
...
; 测试 13
Num++
FileAppend("`n13. 参数数量错误`n", LogFile, "UTF-8")
try {
    result := SubStr("test")
} catch as e {
    LogError(e, "Exit")
...
; 测试 14
Num++
FileAppend("`n14. 无效数学运算`n", LogFile, "UTF-8")
try {
    result := Log(0)
} catch as e {
    LogError(e, "Exit")
...
; 测试 15
Num++
FileAppend("`n15. 数组越界`n", LogFile, "UTF-8")
try {
    arr := [1, 2, 3]
    value := arr[100]
} catch as e {
...
; 测试 16
Num++
FileAppend("`n16. 数组删除错误`n", LogFile, "UTF-8")
try {
    arr := [1, 2, 3]
    arr.RemoveAt(10)
} catch as e {
...
; 测试 17
Num++
FileAppend("`n17. 自定义值错误`n", LogFile, "UTF-8")
try {
    throw ValueError("测试")
} catch as e {
    LogError(e, "Exit")
...
; 测试 18
Num++
FileAppend("`n18. 自定义类型错误`n", LogFile, "UTF-8")
try {
    throw TypeError("测试")
} catch as e {
    LogError(e, "Exit")
...
; 测试 19
Num++
FileAppend("`n19. 自定义内存错误`n", LogFile, "UTF-8")
try {
    throw MemoryError("测试")
} catch as e {
    LogError(e, "Exit")
}
...
; 测试 20
Num++
FileAppend("`n20. 自定义未设置错误`n", LogFile, "UTF-8")
try {
    throw UnsetError("测试")
} catch as e {
    LogError(e, "Exit")
