...
g := Gui("+Resize", "HTML Editor - Dark Theme")
...
htmlFile := A_ScriptDir "\editor_ui.html"
FileAppend(GetEditorHTML(), htmlFile, "UTF-8")
...
.glass-input { background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; color: #fff; padding: 7px 10px; font-size: 12px; outline: none; width: 100%; }
select.glass-input { appearance: none; cursor: pointer; }
select.glass-input option { background: #2d1b69; color: #fff; }
.btn { border: none; border-radius: 8px; padding: 7px 16px; font-size: 11px; cursor: pointer; font-family: inherit; }
...
.hotkey-display { background: rgba(103,126,234,0.2); border: 1px solid rgba(103,126,234,0.4); border-radius: 8px; color: #b3c6ff; padding: 6px 14px; font-size: 13px; font-weight: 600; cursor: pointer; min-width: 60px; text-align: center; }
...
.key-table input { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.12); border-radius: 6px; color: #fff; padding: 6px 8px; font-size: 11px; width: 100%; outline: none; }
...
.key-table .row-btn { background: none; border: none; color: rgba(255,255,255,0.3); cursor: pointer; font-size: 14px; padding: 2px 4px; border-radius: 4px; }
...
.sub-tab { flex: 1; background: transparent; border: none; color: rgba(255,255,255,0.4); padding: 6px 8px; border-radius: 6px; font-size: 10px; cursor: pointer; font-family: inherit; text-align: center; }
...
.preview-arrow { color: rgba(255,255,255,0.2); }
.preview-time { color: rgba(255,255,255,0.3); font-size: 8px; }
.quick-actions { display: flex; gap: 4px; }
.quick-btn { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; color: rgba(255,255,255,0.5); padding: 3px 8px; font-size: 9px; cursor: pointer; font-family: inherit; }
...
.key-pick-btn { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; color: rgba(255,255,255,0.7); padding: 4px 2px; font-size: 9px; cursor: pointer; text-align: center; font-family: inherit; }
.key-pick-btn:hover { background: rgba(103,126,234,0.3); color: #c5caff; }
...
<div class='panel glass' id='mainPanel'>
    ...
    <div class='section' id='keysSection'>
      ...
      <div id='modeContent'>
        <table class='key-table'>
          <thead><tr><th style='width:90px;'>按键</th><th style='width:80px;'>间隔 (ms)</th><th style='width:30px;'></th></tr></thead>
          <tbody>
            <tr><td><input class='key-input' value='Space' onclick='openKeyPicker(this)' readonly></td><td><input class='interval-input' value='50' type='number' min='10'><span class='unit'>ms</span></td><td><button class='row-btn del' onclick='showToast(&quot;已删除&quot;)'>x</button></td></tr>
            <tr><td><input class='key-input' value='1' onclick='openKeyPicker(this)' readonly></td><td><input class='interval-input' value='100' type='number' min='10'><span class='unit'>ms</span></td><td><button class='row-btn del' onclick='showToast(&quot;已删除&quot;)'>x</button></td></tr>
            <tr><td><input class='key-input' value='2' onclick='openKeyPicker(this)' readonly></td><td><input class='interval-input' value='100' type='number' min='10'><span class='unit'>ms</span></td><td><button class='row-btn del' onclick='showToast(&quot;已删除&quot;)'>x</button></td></tr>
          </tbody>
        </table>
        <button class='btn btn-ghost' onclick='showToast(&quot;已添加按键&quot;)' style='margin-top:6px;font-size:10px;padding:4px 10px;'>+ 添加按键</button>
        <div class='form-row' style='margin-top:10px;'><span class='label'>长按键</span><div class='field'><input class='glass-input' placeholder='如 Shift, Ctrl'></div></div>
        <div class='form-row'><span class='label'>长按模式</span><div class='field'><select class='glass-input' style='width:120px;'><option value='continuous'>持续按住</option><option value='periodic'>周期按压</option></select></div></div>
      </div>
    </div>
    <div class='section'>
  ...
  <div class='panel-footer'>
    <div class='quick-actions'>
      <button class='quick-btn' onclick='showToast(&quot;已导入&quot;)'>导入</button>
      <button class='quick-btn' onclick='showToast(&quot;已导出&quot;)'>导出</button>
      <button class='quick-btn' onclick='showToast(&quot;已重置&quot;)'>重置</button>
    </div>
    <div style='display:flex;gap:8px;'>
      <button class='btn btn-ghost' onclick='window.external.onCancel()'>取消</button>
...
<div class='key-picker-overlay' id='keyPickerOverlay'>
  <div class='key-picker glass' id='keyPicker'>
    <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;'>
      ...
      <button class='btn' style='background:rgba(255,255,255,0.1);color:#fff;width:24px;height:24px;font-size:10px;padding:0;' onclick='closeKeyPicker()'>X</button>
    </div>
