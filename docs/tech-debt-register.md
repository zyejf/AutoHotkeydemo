...
| TD-025 | F 基线漂移 | 图谱基线 `graph-baseline.json` 的 `generated_at` 是 **2026-09-13**，此后历次改动（TD-002/003/004…）累积出**未逐项核实**的结构漂移：AHK 101→**100** 文件、294→**292** 边、9→**7** 孤点；JS 5→**6** 文件、9→**11** 边。目前只是 WARNING（不阻塞），但其中 JS 的文件数与边数是**增加（放宽方向）** —— 按「棘轮只收紧不放宽」，未核实前不得吸收进基线。TD-009 更新基线时刻意绕开了它们 | 3 | 2 | 3 | 2 | 3 | 1.4 | **4.1** | P2 | 待排期（2026-09-16 由 TD-009 更新基线时发现） | 4 |

**统计**：P0 **8** 项（**全部完成**）｜P1 **8** 项（**全部完成**）｜P2 **9** 项（其中 1 项已完成）｜合计 **25** 项。
