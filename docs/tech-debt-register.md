...
| TD-023 | A 代码坏味道 | 两个函数超过 `too_many_lines` 阈值：`validator.rs::validate_mode_data` **294 行**、`group_service.rs::register_hotkey` **104 行**。前者是「按 mode 分派的子校验器集合」，切分要先把每个 mode 的子校验抽成函数并对齐错误文案 | 3 | 3 | 3 | 3 | 5 | 1.4 | **3.8** | P2 | **部分完成**（2026-09-16：`validate_mode_data` 已拆完 —— 304 行 → 1 个分派器 + 10 个 `validate_*` + `validate_hybrid_subgroups`，每个 20 行内，文件头 `#[allow]` 已删。6 个「按键 + 时间序列」模式共用 `check_keys_and_timings` + `KeysTimingNaming`（4 个命名参数打包成结构体，否则撞 `too_many_arguments` —— 该 lint **把 `self` 算进去**）。**过程中发现：这 10 个 mode 的错误文案此前一条测试都没有**（全仓 grep 零命中）—— 改错文案、或把某个 mode 分支接到别的 `ModeData` 变体上，整个测试套件都是绿的。兜底办法：先用临时差量对照（从 `git show HEAD:` 取回改动前的原函数，11 modes × 15 样本 = 165 组输入的 `valid` / 错误条数 / `(group_id, field, message)` / `warnings` 全一致）证明行为未变，再把它固化成常驻特征测试 7 条 `mode_data_msg_*`（`validator.rs` 55 → 62）。阳性对照 6 组注入（mode 分支接错变体 / 子组文案写错 / 顶层字段名写错 / `joystick_hold` 告警改成错误 / `_` 分支吞掉未知模式 / 空子组 `field` 写错）全部被抓住。⚠️ 拆完后新写的那条测试自己撞了 `too_many_lines`（241 行）—— 按关注点拆成 7 条，没给它加 `#[allow]`。剩余 `group_service.rs::register_hotkey` 104 行） | 3 |
...
**统计**：P0 **8** 项（**全部完成**）｜P1 **8** 项（**全部完成**）｜P2 **11** 项（2 项已完成、**5 项部分完成**、1 项豁免、3 项待排期）｜合计 **27** 项。
