...
| TD-021 | F 文档缺失 | `missing_errors_doc` / `missing_panics_doc` 两条 lint **有意暂缓**：纯逻辑 crate 有 59 个公开函数缺 `# Errors` / `# Panics` 小节；`src-tauri` 另有同类文档债 476 项（与 `doc_markdown` 合并统计）。属文档工程，与「静态分析收紧」分开做 | 2 | 2 | 3 | 1 | 3 | 1.0 | **4.6** | P2 | **部分完成**（2026-09-16 实测复核：台账里的「59」**准确无误**，实测恰好 59 条 = 55 `# Errors` + 4 `# Panics`。已处置 16 条，**剩余 43 条**在 `asd-application`。① **`asd-domain/src/traits.rs` 4 条已补** —— 契约定义文件优先级最高，文档写明「默认实现恒定 `Err` 表达的是**不具备该能力**，调用方据此降级而不是当失败上报」。② **`backup_service.rs` 8 条已补**（读函数体得出，未凭签名猜）。③ **4 条 `# Panics` 判定为不修并转为显式豁免**：全在 `asd-test-harness`，panic 源是 `.lock().unwrap()` / `TempDir::new().unwrap()`，表达的是「用例自身出意外」而非「调用方该处理的失败」，补契约反而是误导性文档。④ 两条 lint 已从 `allow` 改为 **`warn`**，并对已补齐的范围立刻生效。**推进机制（比补文档本身更重要）**：不放 crate 级豁免，改为**未补齐的文件各自在文件头 `#![allow]`**，补完一个文件就删掉那一行、该文件立即受保护 —— 已用阳性对照验证：往 `backup_service.rs` 加一个未写契约的 `pub fn` 立刻变红。⚠️ 补文档过程中发现两条**真信号**（已写进 `backup_service.rs` 模块文档）：`AppState::read_config` 的 `Result` **恒定返回 `Ok`**，只是为 API 稳定保留返回类型，而 `clippy::unnecessary_wraps` 报不出来（默认不检查导出函数）；`save_config_atomic` 写盘失败时**会先回滚内存**再返回错误） | 3 |
