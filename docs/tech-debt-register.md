...
| TD-027 | F 文档缺失 | `ConfigLoadError::FileNotFound` 的**变体名与语义不符**：它是 `fs::read_to_string` **全部**错误的兜底映射，权限不足 / 路径其实是目录 / 内容非 UTF-8 都会落到它，而 `#[error]` 模板写死了「配置文件不存在: {0}」→ 实际消息可能是「配置文件不存在: Permission denied (os error 13)」。当前**唯一**调用方（`src-tauri/src/lib.rs` 启动时加载配置）对所有错误一视同仁（记日志 + `Config::default()`），所以还没有造成实际后果；但它是个**潜伏陷阱** —— 任何人据此「拿到 FileNotFound 就是首次启动，写一份默认配置」，就会在真实故障时把排查线索覆盖掉 | 2 | 2 | 3 | 2 | 3 | 1.4 | **3.7** | P2 | 待排期（2026-09-16 补 TD-021 的 `# Errors` 时发现，已在 `ConfigRepository::load_from_file_checked` 文档里写明这个陷阱与规避办法。两种修法：① 新增 `IoError(#[source] io::Error)` 变体、把 `FileNotFound` 收窄到 `ErrorKind::NotFound` —— 语义最正，但改公开枚举是 breaking change；② 只把 `#[error]` 模板改成「配置文件无法读取: {0}」—— 零风险，代价是丢掉「是哪种失败」的表达力。推荐 ①，与模块文档里已记的那条「`Result<_, String>` 统一到 `ConfigLoadError`」的债**一起做**，只改一处不划算） | 3 |

**统计**：P0 **8** 项（**全部完成**）｜P1 **8** 项（**全部完成**）｜P2 **11** 项（**3 项已完成**、4 项部分完成、1 项豁免、3 项待排期）｜合计 **27** 项。
