...
>
> ⚠️ **C2 的「测试未接入执行」计数仍会把这 8 个算进去** —— 它判的是「能否从
> `run_all_tests.ahk` 的 `#Include` 图到达」，不认外部 runner。所以 G3e 输出里看到
> `C2 当前 8 项` 是**预期状态**，既不要以为是漏接，更不要据此去删这些脚本 ——
> 它们正由 G3g 逐个执行（CI 实测 8/8）。
...
| C11 | **打包资源清单同步**（**不做棘轮**）：以 `asd-tauri/src-tauri/ahk_executor/*.ahk` 里的 `#Include` 为事实源，反向校验 `tauri.conf.json` 的 `bundle.resources` 必须覆盖全部被引用的**同目录** `.ahk`。起因（2026-09-18）：resources 漏了 `high_res_clock.ahk`，而 `sender.ahk:16` / `joystick.ahk:18` 都要 include 它 —— **任何干净的打包构建**（CI 全新 checkout 或用户拿到安装包，此时没有本地编译的 `asd_executor.exe` 兜底，走便携模式）执行器都会**启动即崩**；开发机永远测不出来，与 TD-046 同家族。⚠️ 打击面刻意**只管同目录**：跨目录引用（`../domain/x.ahk`）各有自己的打包口径，混进来只会制造无法归因的噪声。目录里**从未被引用**的 `.ahk` 只提示不判失败（可能是入口脚本，如 `executor.ahk` 由 `asd_executor.bat` / Rust 直接拉起，本就不该有入边）—— 要人判，机器判了就是误报 |
| C12 | **正式配置不得开调试端口**（**不做棘轮**）：`asd-tauri/src-tauri/tauri.conf.json` 里**不得出现** `--remote-debugging-port`；E2E 专用配置 `tauri.e2e.conf.json` 是**合法例外、明确不查**。起因（2026-09-18）：E2E 要靠该开关才能让 msedgedriver 挂上 WebView2，但它一旦混进正式配置，**分发给用户的 release 产物就带着远程调试端口** —— 是安全红线而非配置噪声。两份配置长得几乎一样、只差一个字段，人眼分不出来，故由机器守。⚠️ 扫**原始文本**而非 JSON 解析后的值：开关可能出现在任何字符串值里，解析后遍历会漏。文件不存在 / 读不了按**失败**处理（正式配置都不在了说明路径变了，守卫必须响） |
