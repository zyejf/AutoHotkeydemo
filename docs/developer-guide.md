...
| `--bench` | 守什么 | 被测生产代码 | K |
|---|---|---:|---:|
| `prod_escape`（默认） | T1 · JSON 转义快路径 | `JSONSerializer._EscapeString` | **1.4**（p50 自 2026-09-15 起也是归一化值） |
...
断言 `中位数 ≤ 基线 × K`（`baselines.json` 里每个 bench 各自的 `k`：两个 bench 目前都是
**1.4**，`warn_k` 都是 **1.2** —— 见下方两节「为什么要归一化」）。

- **只设上界**：变快不会失败 —— 只有变慢超过 K 倍才红；
- **多轮中位数**：p50 滤单次离群，中位数再滤整轮异常（宿主抖动常整轮偏移）；
- 另有 **WARN 带**（`warn_k=1.2`）：进入 1.2×~1.4× 只告警不阻断。
...
**基线状态**：归一化改变了 p50 语义，旧基线作废，`metrics` 已清空为 pending-ci（此期间门禁
报「新指标（无基线）」并放行），待两次独立 CI 运行重取后按需校准 K。
