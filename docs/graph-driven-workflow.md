...
| `asd-tauri/docs/test-map.md` | **一切测试数量**（函数数、用例数、套件数）+ **3 个纯逻辑 crate 的覆盖率基线/棘轮**。⚠️ 覆盖率**分口径**：全仓/含 `src-tauri` 的对照表与测量命令在 `docs/developer-guide.md` §4.6.1.3；两个口径的数字不可混用 | 架构规则、流程 |
