...
;   每行：`metric,<name>,p50=..,p95=..,max=..,min=..,mean=..,n=..,len=..,us_per_call=..`
...
;   判据用 p50（批耗时，毫秒），us_per_call 仅供人工复核量级。
; =================================================================
...
RunCase(name, s, calls := 0) {
    global BATCH_CALLS, BATCH_ROUNDS
    if calls = 0
        calls := BATCH_CALLS
    samples := []
    i := 1
    ...
    BenchWrite("metric," . name
        . ",p50=" . Round(p50, 4)
        . ",p95=" . Round(Pct(sorted, 0.95), 4)
        . ",max=" . Round(sorted[sorted.Length], 4)
        . ",min=" . Round(sorted[1], 4)
        . ",mean=" . Round(Mean(sorted), 4)
        . ",n=" . sorted.Length
        . ",len=" . StrLen(s)
        . ",calls=" . calls
        . ",us_per_call=" . Round(p50 * 1000 / calls, 4))
}
