...
4. **补齐 watchdog 晋级留痕**（发现 #5b，Cody 提出、我已独立核实）：按**最严**那套判据（本机与 CI **各连续 ≥10 次**全绿、含 `--test-threads=1`）补齐留痕，或显式决议改判据并同步三处；或暂把该组单独成步并保留 `continue-on-error` 至观测期完成。⚠️ **三套判据冲突未解决前，先统一口径再谈"是否已满足"** —— 否则「满足了 3-5 次」与「满足了 ≥10 次」会被各自援引，留痕本身失去意义。**（team-lead 汇总报告已列为 finding #11，指派 Tessa，P0，0.5 人日）**
...
- v5（2026-09-19）：**消除报告内部自相矛盾** —— §7.2 P0 第 4 项此前仍写「补连续 3-5 次」「两处判据」，与已升级为「三套冲突 / 最严 ≥10 次」的发现 #5b 不一致（发现表改了、行动项没跟着改）。已改为按最严判据补齐，并补入「先统一口径再谈是否已满足」。<br>⚠️ 这与 Cody 在其 §6.2 自行修掉的同类问题（建议写 3-5 次却自列 ≥10 次判据）是同一种错误：**改了结论没同步行动项**。留痕以防两份报告被交叉引用时出现矛盾口径。
