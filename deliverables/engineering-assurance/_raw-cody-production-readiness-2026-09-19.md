...
**根因分两端，防法也不同**（与 tessa 交叉复核时共同归纳，她补了输出端一半）：

| 端 | 机理 | 表现 | 防法 |
|---|---|---|---|
| 取数端 | 污的是**输入** | 字面量 vs 正则的假阴性、过滤跑法当全量 | 先确认命令语义，再采信其输出 |
| 输出一致性端 | 污的是**引用一致性** | 结论升级了，行动项/汇总表/给别人的消息没跟着改 | **结论变更后必须回扫所有下游引用点** |

本项目本轮两端各踩一次：我在取数端（上表两处）；tessa 在输出一致性端——她的发现表已升级为「三套冲突 / 最严 ≥10 次」，但 §7.2 的 P0 行动项仍停在旧版「补 3-5 次」，且她在消息里给了新结论却未同步自己的 P0 列表。两份报告若被交叉引用就会出现口径打架，故第六节末尾的「四要素」以双向核对后的版本为准。
...
**两条独立反证**（不依赖「有无留痕」，单看代码现状即可判定摘除动作未按其自定规则执行）：

- **反证 1 —— 判据段没删**：`watchdog_integration_tests.rs:24` 明文要求「摘的同时必须把本段判据一并删除或改写，不许留下过期说明」，而该判据段（`:14-26`）**至今仍在文件里**。即摘除动作连它自己写的规则都没执行。
- **反证 2 —— 替身优先被绕过**：同段 `:25-26` 第 5 条要求「若能用替身（不杀真实进程、只断言调用序列）覆盖同一契约，**优先用替身**，把真实进程用例降级为可选的手动验证」。而这 15 条恰恰是真实派生 `cmd.exe` + 执行 `taskkill` 清理的真实进程用例（`:22` 亦要求拿不到外部依赖时须显式打印「跳过」），直接摘 ignore 进 CI，与第 5 条同样拧着。

> 行号校准：`≥10 次` 在 `:23-24`、`须删除判据段` 在 `:24`、第 5 条（替身优先）在 `:25-26`、判据段整体为 `:14-26`。tessa 初版引作 24-25 / L25 / L15-27（各偏 1），已双向核对修正。
